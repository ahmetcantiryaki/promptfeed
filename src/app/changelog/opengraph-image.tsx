...
/**
 * Render the latest changelog issue as a magazine-cover PNG.
 *
 * Pattern follows the Next.js 16 docs example for opengraph-image:
 *   - default Node.js runtime (no `runtime = "edge"` export)
 *   - local TTF fonts loaded via readFile + process.cwd()
 *   - JSX restricted to Satori-supported CSS (flexbox only, no
 *     `inline-block`, no `grid`)
 *
 * The route is statically optimized — Next regenerates on each deploy
 * because the CHANGELOG data is read at module load time.
 */
export default async function Image() {
  ...
  return new ImageResponse(
    (
      ...
      >
        {/* Top masthead — hairline rules, three-up labels */}
        <div
          style={{
            display: "flex",
        ...
        {/* Body — huge issue numeral on the left, headline column on the right */}
        <div
          style={{
            display: "flex",
            flex: 1,
