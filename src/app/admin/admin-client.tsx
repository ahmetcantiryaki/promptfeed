  ...
  return (
    <div className="grid min-h-[100dvh] grid-cols-1 lg:grid-cols-[248px_1fr]">
      ...
      <main className="flex min-w-0 flex-col">
        <header className="sticky top-0 z-20 flex h-[60px] shrink-0 items-center justify-between gap-3 border-b bg-surface px-4 sm:px-6">
        ...
        <div className="flex-1 px-4 pb-10 pt-5 sm:px-7 sm:pt-6">
...
function AdminSidebar({ section, onSectionChange, counts }: SidebarProps) {
  return (
    <>
      ...
      <div className="sticky top-0 z-30 flex flex-col gap-2 border-b bg-surface px-3 py-3 lg:hidden">
        <div className="flex items-center justify-between gap-2">
          ...
          >
            <LogoMark height={24} width={112} />
          </Link>
          <Link
            href="/"
            className="inline-flex items-center gap-1 rounded-full border bg-surface px-2.5 py-1 text-[11px] font-medium text-text-muted transition-colors hover:bg-hover hover:text-text"
          >
          ...
          {SECTIONS.map(({ key, label, Icon }) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "inline-flex shrink-0 items-center gap-1.5 rounded-full border px-3 py-1.5 text-[13px] transition-colors min-h-9",
                  active
                    ? "border-text/30 bg-surface-2 font-medium text-text"
      ...
      <aside className="sticky top-0 hidden h-[100dvh] w-[248px] shrink-0 flex-col gap-4 overflow-hidden border-r bg-surface px-3 py-4 lg:flex">
        <Link
          href="/"
          aria-label="Feedlens.ai"
    ...
    <>
      <div className="flex flex-col gap-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <label className="flex min-h-10 w-full items-center gap-2 rounded-[10px] border bg-surface-2 px-3 py-2 sm:w-[420px]">
            <Search
        ...
        <ul className="flex flex-col gap-2 md:hidden">
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <img
                  ...
                  alt=""
                  className="h-14 w-14 shrink-0 rounded-[8px] bg-surface-2 object-cover"
                />
                <div className="flex min-w-0 flex-1 flex-col gap-1">
                  ...
                  <div className="flex flex-wrap items-center gap-1.5 text-[11px] text-text-subtle">
                    {p.prompt_type === "remix" ? (
                      <span className="inline-flex items-center gap-1 rounded-full border bg-surface-2 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-[0.06em]">
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 border-t pt-2">
                <div className="flex min-w-0 flex-col text-[12px]">
                  ...
                  <span className="truncate text-[11px] text-text-subtle">
                    {prettyPlatform(p.platform_slug)} · {p.source_user}
                  </span>
                </div>
                <div className="flex items-center gap-1.5">
                  ...
                  <button
                    ...
                    className="inline-flex h-9 w-9 items-center justify-center rounded-[8px] border bg-surface text-text-muted hover:border-red-500/40 hover:bg-red-500/10 hover:text-red-500"
                    aria-label="Delete"
                  >
          ...
          {filtered.length === 0 ? (
            <li className="rounded-[12px] border bg-surface px-4 py-10 text-center text-[13px] text-text-muted">
              No prompts match that search.
            </li>
          ) : null}
        </ul>

        <div className="hidden overflow-x-auto rounded-[12px] border bg-surface md:block">
...
function UsersPanel({ profiles }: { profiles: ProfileWithEmail[] }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <label className="flex min-h-10 w-full items-center gap-2 rounded-[10px] border bg-surface-2 px-3 py-2 sm:w-[360px]">
        <Search
          className="h-3.5 w-3.5 shrink-0 text-text-subtle"
          strokeWidth={2}
        />
        ...
        {filtered.map((p) => {
          ...
          return (
            <li
              key={p.id}
              className={cn(
                "flex flex-col gap-2.5 rounded-[12px] border bg-surface p-3",
                p.is_banned && "border-red-500/30 bg-red-500/5",
              )}
            >
              <div className="flex items-start gap-2.5">
                ...
                <div className="flex min-w-0 flex-1 flex-col gap-0.5">
                  <div className="flex items-center gap-1 truncate text-[14px] font-medium text-text">
                    {p.display_name ?? p.email ?? "—"}
                    {p.is_verified ? (
                      <BadgeCheck
                        className="h-3.5 w-3.5 shrink-0 text-text"
                        strokeWidth={2}
                      />
                    ) : null}
                  </div>
                  ...
                  <div className="mt-0.5 flex flex-wrap items-center gap-1.5 text-[11px]">
                    {p.is_admin ? (
                      <span className="rounded-full border border-text/30 bg-surface-2 px-2 py-0.5 font-semibold uppercase tracking-[0.06em]">
                        Admin
                      </span>
                    ) : p.is_banned ? (
                      <span className="rounded-full border border-red-500/40 bg-red-500/10 px-2 py-0.5 font-semibold uppercase tracking-[0.06em] text-red-500">
                        Banned
                      </span>
                    ) : (
                      <span className="text-text-subtle">Member</span>
                    )}
              ...
              <div className="flex flex-wrap items-center gap-1.5 border-t pt-2">
                <button
                  ...
                  className={cn(
                    "inline-flex min-h-9 items-center gap-1 rounded-[8px] border px-3 py-1.5 text-[12px] font-medium transition-colors disabled:opacity-50",
                    p.is_verified
                ...
                <button
                  type="button"
                  onClick={() => toggleFlag(p.id, "is_banned", !p.is_banned)}
